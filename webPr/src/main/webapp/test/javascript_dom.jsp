...
function fruit() {
	...
	let cls = document.getElementsByClassName("a");		// 클래스속성(프로퍼티)으로 객체를 찾는다.
	for(let i=0; i<cls.length; i++) {
		let entitycls = cls[i];
		entitycls.style.color = "red";
		entitycls.style.fontSize = "20px";
	}
	return;
}
...
function winOpen() {
	...
	window.open("<%=request.getContextPath()%>/member/memberJointeacher.jsp", "winName", "width=200, hegiht=300;");
...
<button type="button" onclick="fruit()">과일이름 바꾸기</button>
